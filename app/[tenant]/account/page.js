...
export default function AccountPage() {
  ...
  useEffect(() => {
    ...
    getOrdersByCustomer(tenant.id, customerUser.uid).then((data) => setOrders(data.slice(0, 3)))
      ...
      <div className="space-y-6">
        <div className="bg-white border border-gray-200 rounded-2xl p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            {!editing && (
              <button onClick={() => setEditing(true)} className="text-sm text-black hover:underline">Edit</button>
            )}
          </div>
          ...
          {editing ? (
            <form onSubmit={handleSave} className="space-y-3">
              ...
              <div className="flex gap-2">
                <button type="submit" disabled={saving} className="px-4 py-2 text-sm font-medium rounded-xl disabled:opacity-50"
                  style={{ background: 'var(--color-primary)', color: 'var(--color-primary-fg)' }}>
                  {saving ? 'Menyimpan...' : 'Simpan'}
                </button>
                <button type="button" onClick={() => setEditing(false)} className="px-4 py-2 text-sm text-gray-600 border border-gray-300 rounded-xl hover:bg-gray-50">Batal</button>
              </div>
            </form>
          ) : (
            <div className="space-y-3">
              <div><p className="text-xs text-gray-500">Nama</p><p className="text-sm font-medium text-gray-900">{customer?.name || '-'}</p></div>
              <div><p className="text-xs text-gray-500">Email</p><p className="text-sm text-gray-900">{customerUser?.email}</p></div>
              <div><p className="text-xs text-gray-500">WhatsApp</p><p className="text-sm text-gray-900">{customer?.whatsapp || '-'}</p></div>
            </div>
          )}
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href={`/${tenantSlug}/account/orders`} className="text-sm text-black hover:underline">Lihat semua</Link>
          </div>
          {orders.length === 0 ? (
            <div className="text-center py-8">
              <p className="text-sm text-gray-400">Belum ada pesanan</p>
              <Link href={`/${tenantSlug}/collections`} className="inline-block mt-3 text-sm text-black font-medium hover:underline">Mulai belanja →</Link>
            </div>
          ) : (
            <div className="space-y-3">
              {orders.map((order) => (
