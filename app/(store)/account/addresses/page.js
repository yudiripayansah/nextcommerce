...
export default function AccountAddressesPage() {
  ...
  async function load() {
    setLoading(true)
    const data = await getAddresses(customerUser.uid)
    setAddresses(data)
    setLoading(false)
  }
  ...
  async function handleSave(e) {
    ...
    try {
      if (editingId) {
        await updateAddress(editingId, customerUser.uid, form)
      } else {
        await addAddress(customerUser.uid, form)
      }
      await load()
      closeForm()
    } finally {
            ...
            <form onSubmit={handleSave} className="space-y-3">
              ...
              <div className="flex gap-2 pt-1">
                <button
                  type="submit"
                  disabled={saving}
                  className="px-5 py-2 text-sm font-medium rounded-xl disabled:opacity-50"
                  style={{ background: 'var(--color-primary)', color: 'var(--color-primary-fg)' }}
                >
                ...
                <button
                  type="button"
                  onClick={closeForm}
                  className="px-5 py-2 text-sm text-gray-600 border border-gray-300 rounded-xl hover:bg-gray-50"
                >
                  Batal
                </button>
              </div>
                ...
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="flex gap-2 flex-shrink-0">
                    <button
                      ...
                      className="text-xs text-gray-600 hover:text-black border border-gray-200 px-3 py-1.5 rounded-lg hover:bg-gray-50"
                    >
                      Edit
                    </button>
                    <button
                      onClick={() => setConfirmDelete(addr.id)}
                      className="text-xs text-red-500 hover:text-red-700 border border-red-100 px-3 py-1.5 rounded-lg hover:bg-red-50"
                    >
                      Hapus
                    </button>
                  </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-2xl shadow-xl p-6 w-80 mx-4">
            ...
            <div className="flex gap-3">
              <button onClick={() => setConfirmDelete(null)} className="flex-1 px-4 py-2 text-sm font-medium text-gray-700 border border-gray-300 rounded-xl hover:bg-gray-50">
                Batal
              </button>
              <button onClick={() => handleDelete(confirmDelete)} className="flex-1 px-4 py-2 text-sm font-medium text-white bg-red-600 rounded-xl hover:bg-red-700">
                Hapus
              </button>
            </div>
          </div>
